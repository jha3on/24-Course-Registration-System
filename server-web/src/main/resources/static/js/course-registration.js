...
document.addEventListener("DOMContentLoaded", function (event) {
   ...
   * 수강 신청 조회
   */
  course.onGetCourseRegistrations().then(() => course.onGetCourseRegistrationCarts().then());
  ...
  document.addEventListener("click", function (event) {
    ...
    if (event.target.classList.contains("course-registration-create-button")) {
      ...
      course.onCreateCourseRegistration(courseId);
    }
    ...
    if (event.target.classList.contains("course-registration-delete-button")) {
      const courseId = event.target.closest("tr").getAttribute("data-key");
      course.onDeleteCourseRegistration(courseId).then(() => location.reload());
    }
    ...
    if (event.target.classList.contains("course-registration-cart-create-button")) {
      const courseId = event.target.closest("tr").getAttribute("data-key");
      course.onCreateCourseRegistration(courseId).then();
    }

    /**
     * 모달 닫기 이벤트 등록
     */
    if (event.target === document.querySelector(".js.course-registration-close-button")) {
...
const course = {
    ...
    result.forEach((item, i) => {
      ...
      document.querySelector(".js.course-search-details-table-content").insertAdjacentHTML("beforeend", `
        <tr data-key="${item.courseId}">
          ...
          <td>${item.courseRegistrationCount}</td>
          ${isUserCourseId === -1
            ? `<td><button class="course-registration-create-button js" type="button">신청</button></td>`
            : `<td><button class="course-registration-create-button js" type="button" disabled>신청완료</button></td>`
          }
        </tr>
      `);
    });
  ...
  onGetCourseRegistrations: async function () {
    course.onClearCourseRegistrations();

    const { code, result } = await client.useGetByCourseRegistrations();
    ...
    if (result.length !== 0) {
      result.forEach((item, i) => {
        ...
        document.querySelector(".js.course-registration-details-table-content").insertAdjacentHTML("beforeend", `
          <tr data-key="${item.courseId}">
            <td>${item.collegeName}</td>
            <td>${item.collegeDivisionName}</td>
            <td>${item.collegeDepartmentName}</td>
            ...
            <td>${item.courseRegistrationCountLimit}</td>
            <td>${item.courseRegistrationCount}</td>
            <td><button class="course-registration-delete-button js" type="button">취소</button></td>
          </tr>
        `);
      });
    }
  ...
  onCreateCourseRegistration: function (courseId) {
    client.usePostByCourseRegistration(courseId).then(({ code }) => {
      if (code !== "+100") {
        alert("신청 실패");
        return;
      }

      const stompClient = Stomp.over(new SockJS(`http://localhost:8001/ws`));

      stompClient.connect({}, function () {
        document.querySelector(".js.modal-overlay").style.display = "flex";
        stompClient.subscribe(`/ws-sub/course-registration/${localStorage.getItem("user_id")}`, (payload) => {
          document.querySelector(".js.modal-overlay").style.display = "none";
        ...
        stompClient.subscribe(`/ws-sub/course-registration-order/${localStorage.getItem("user_id")}`, (payload) => {
          if (isFirstPayload) {
          ...
          course.onUpdateCourseRegistrationQueue(waitingCount, waitingCurrentCount);
        });
      });
    });
  },
  ...
  onUpdateCourseRegistrationQueue: function (waitingCount, waitingCurrentCount) {
    document.querySelector(".js.course-registration-progress-gauge").style.width = `${((waitingCount - waitingCurrentCount) / waitingCount) * 100}%`;
    document.querySelector(".js.course-registration-waiting-time-minute").innerText = Math.floor(waitingCurrentCount * 0.3 / 60);
    document.querySelector(".js.course-registration-waiting-time-second").innerText = Math.floor(waitingCurrentCount * 0.3 % 60);
    document.querySelector(".js.course-registration-waiting-count-number").innerText = waitingCurrentCount;
  },

  // onCreateCourseRegistration: function (courseId) {
  //   let totalCount;
  //   let currentCount;
  //   const sockJs = new SockJS(`http://localhost:8001/ws`);
  ...
  //       if (isFirstPayload) {
  //         isFirstPayload = false;
  //         totalCount = currentCount = payload.body;
  //       } else {
  //         currentCount = payload.body;
  //       }
  //       course.onUpdateCourseRegistrationQueue(totalCount, currentCount);
  //     });
  //   });
  //
  //   client.usePostByCourseRegistration(courseId).then();
  // },
  ...
  onDeleteCourseRegistration: async function (courseId) {
    const { code } = await client.useDeleteByCourseRegistration(courseId);
